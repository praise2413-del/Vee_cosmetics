            ...
            {filteredProducts.map((product) => (
              ...
              >
                ...
                <div className="relative aspect-square w-full overflow-hidden bg-rose-light/10">
                  ...
                  {/* Category Tag */}
                  <span className="absolute top-4 left-4 bg-white/90 dark:bg-black/80 backdrop-blur-sm text-text-base text-[10px] font-bold tracking-widest uppercase px-3 py-1 rounded-full border border-border-base">
                    {product.category}
                  </span>
                  ...
                  <button 
                    ...
                    className="absolute top-4 right-4 p-2 rounded-full bg-white/90 dark:bg-black/80 backdrop-blur-sm shadow-sm transition-all duration-200 border border-transparent hover:border-gold/40 cursor-pointer"
                  >
                    <Heart 
                  ...
                  <div className="absolute inset-0 bg-black/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                    <span className="bg-gold text-white text-xs uppercase tracking-wider font-semibold py-2 px-4 rounded-full shadow-md transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                      Quick View
                    </span>
                  </div>
                </div>
                ...
                <div className="p-6 flex-grow flex flex-col justify-between space-y-4 text-left">
                  <div className="space-y-1.5">
                    <h3 className="font-serif text-lg font-bold text-text-base group-hover:text-gold transition-colors duration-200 line-clamp-1">
                      {product.name}
                    </h3>
                  ...
                  <div className="pt-2 flex items-center justify-between border-t border-border-base/50">
                    <span className="font-bold text-lg text-gold tracking-wide">
                      {product.price} <span className="text-xs font-semibold text-text-base">TSh</span>
                    </span>
                    <a
                      ...
                      className="p-2.5 rounded-full bg-rose-light dark:bg-rose-light/10 text-rose-primary hover:bg-rose-primary hover:text-white transition-colors duration-300"
        ...
        {selectedProduct && (
          <div className="fixed inset-0 z-50 overflow-y-auto bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
            <motion.div 
              ...
              className="bg-card-bg w-full max-w-3xl rounded-3xl overflow-hidden shadow-2xl border border-border-base relative"
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2">
                ...
                <div className="relative aspect-square md:h-full bg-rose-light/10">
                  <img 
                ...
                <div className="p-8 flex flex-col justify-between text-left space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="space-y-1">
                      <span className="text-xs font-semibold text-gold uppercase tracking-widest">{selectedProduct.category}</span>
                      <h2 className="font-serif text-2xl md:text-3xl font-bold text-text-base leading-tight">
                        {selectedProduct.name}
                      </h2>
                    </div>

                    <div className="inline-flex items-center space-x-1.5 text-xs text-text-muted font-medium bg-rose-light/50 dark:bg-rose-light/5 px-2.5 py-1 rounded-md">
                    ...
                    <div className="space-y-2">
                      <h4 className="text-xs font-bold uppercase tracking-wider text-text-base">Product Highlights:</h4>
                      <ul className="space-y-1.5">
                        {selectedProduct.features?.map((feat, index) => (
                          <li key={index} className="flex items-center space-x-2 text-xs text-text-muted">
                  ...
                  <div className="pt-6 border-t border-border-base/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex flex-col">
                      <span className="text-xs text-text-muted">Price:</span>
                      <span className="font-bold text-2xl text-gold tracking-wide">
                        {selectedProduct.price} <span className="text-sm font-semibold text-text-base">TSh</span>
                      </span>
                    </div>
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center justify-center px-6 py-3 rounded-full bg-rose-primary text-white text-sm font-medium hover:bg-rose-hover transition-colors shadow-md hover:shadow-lg cursor-pointer uppercase tracking-wider gap-2 border border-transparent"
                    >
